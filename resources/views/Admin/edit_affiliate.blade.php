...
<section class="content-header">
    <h1>
	    <span class="text-capitalize">Edit Affiliate</span>
	  </h1>
    <ol class="breadcrumb">
	...
	<a href="{{ url('admin/manageAffiliate') }}"><i class="fa fa-angle-double-left"></i> Back to <span class="text-lowercase">manage affiliate </span></a><br><br>
        <div class="box">
            <?php //echo '<pre>'; print_r($UserVal);die; ?>
                <form method="POST" action="{{url('admin/updateAffiliate')}}" id="admin_add_aff" accept-charset="UTF-8">
                    {{ csrf_field() }}
                    ...
                    <div class="box">
                        ...
                        <div class="box-body">
							...
							<div class="form-group col-md-12">
                                ...
                                <input type="text" id="phone" name="phone" value="{{ $result->phone }}" class="form-control">
                                @if ($errors->has('phone'))
                                    <div class="error">
                                        {{ $errors->first('phone') }}
                                    </div>
                            ...
                            <div class="form-group col-md-12">
                                ...
                                <input type="text" id="email" name="email" value="{{ $result->email }}" class="form-control" readonly>
                                @if ($errors->has('email'))
                                    <div class="error">
                                        {{ $errors->first('email') }}
                                    </div>
                            ...
                            <div class="form-group col-md-12">
                                ...
                                <input type="text" id="commission_percentage" name="commission_percentage" value="{{ $result->commission_percentage }}" class="form-control">
							...
							<div class="input_fields_wrap">
								...
								@if( $result->URL_links!='')
									@foreach(explode(',', $result->URL_links) as $key=>$links)
										<div class="form-group col-md-12">
											<label>URL {{++$key}}<span class="red">*</span></label>
											<input type="text" id="URL[]" name="URL[]" value="{{ $links }}" class="form-control"><a href="#" class="remove_field">Remove</a>
										</div>
									@endforeach
								@endif
							</div>
...
<script src="{{ asset('public/js/jquery.min.js')}}"></script>
 <script>
	  $(document).ready(function() {
		...
		//Add more fie
		var max_fields      = {{$countURL}}+1; //maximum input boxes allowed
