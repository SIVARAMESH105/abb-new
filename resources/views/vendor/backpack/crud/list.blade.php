...
@section('header')
	<section class="content-header">
	  <h1>
		@if(isset($_SESSION['coachName']))
			<span class="text-capitalize">{{ $crud->entity_name_plural.' for '.$_SESSION['coachName']}}</span>
		@else
		...
		@php
			...
			echo count($url); exit;
			if($url[5] != 'viewCoachRosters') {
				$pieces = explode(" ", $crud->entity_name_plural);
				$string_val = 'Manage '.$pieces[1];
			} */
		@endphp
	  <ol class="breadcrumb">
		...
		@php
		/* @if($url[5] != 'viewCoachRosters')
			<li class="active">{{ $pieces[1] }} {{ trans('backpack::crud.list') }}</li>
		@endif */
		@endphp
	  </ol>
	</section>
	...
	<script type="text/javascript">
      ...
      function register_delete_button_action() {
        ...
        $("[data-button-type=delete]").click(function(e) {
          ...
          var rest = delete_url.substring(0, delete_url.lastIndexOf("/") + 1);
          var restUrl= rest.replace(/\/$/, "");
          var last = restUrl.substring(restUrl.lastIndexOf("/") + 1, restUrl.length);
          if(last == 'manageLocations'){
              var messageConfirm =  confirm("{{ trans('backpack::crud.delete_confirm_location') }}");          
          }else{
              var messageConfirm =  confirm("{{ trans('backpack::crud.delete_confirm') }}")
          }
