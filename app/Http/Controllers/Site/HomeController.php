...
class HomeController extends Controller
{
    ...
    public function homePage(){
		...
		$data['list_cities'] = $locationObj->listCityDetails();
        $pageName = 'home';  
		$cmsObj = new Cmsfornt();
    ...
    public function page($pageName){   
		...
		switch($pageName) {
			...
			case 'schedule':
			    ...
			    $data['camp_startdate'] = $campsObj->getCampsStartDate();
			  	if(isset($_GET['affcode'])) {
					...
					$data['affcode'] = Session::put('affcode', $affcode);
				}
			...
			break; 
			default:
                if (count($data['pageContent']) == 0 || $data['pageContent'][0]->title=="privacy") {
                    $pageName = "404";
                    $data['pageContent'] = $cmsObj->getPageDetails($pageName);
                }
	...
	public function registerSave(Request $request){
		...
		return redirect('camp/register/'.$request->input('camp_id'))->with('error', 'Already this camp registered.')->withInput();
		}
	}
    ...
    /**
	    *Purpose  :For Search result display of both cms,geolocation camp and Camp page
	    *Parameter:search data
	    *Return   :Array of cms content,and camp information
     **/

    public function ajaxSearchByContent(Request $request){
		//Replace Dash by add space symbol
		$dashData = str_replace("-", " ", $request->input("search"));
		//Replace Space by add + symbol
		$searchData = str_replace(" ", "+", $dashData);
	    $campObj = new Camps();
	    //Array of Data passed.
	    $strExplode = explode("+",$searchData);
	    $data['campsAndCMS'] = $campObj->getSearchCampDetails($strExplode);
        
        $data['title'] = "Search Result";
        //For caps lock purpose word shown ucfirst function added
